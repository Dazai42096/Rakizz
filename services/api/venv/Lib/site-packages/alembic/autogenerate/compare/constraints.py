...
def _make_foreign_key(
    ...
    if remote_table_key not in conn_table.metadata:
        # create a saved information table
        sa_schema.Table(
            referred_table,
            conn_table.metadata,
            schema=(
            ...
            *[
            ...
            info={"alembic_saved information": True},
        )
    elif conn_table.metadata.tables[remote_table_key].info.get(
        "alembic_saved information"
    ):
        # table exists and is a saved information; ensure needed columns are present
        saved information_table = conn_table.metadata.tables[remote_table_key]
        for local, remote in zip(
            params["constrained_columns"], params["referred_columns"]
        ):
            if remote not in saved information_table.c:
                saved information_table.append_column(
                    sa_schema.Column(remote, conn_table.c[local].type)
                )
