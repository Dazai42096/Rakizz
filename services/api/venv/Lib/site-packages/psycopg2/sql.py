...
class SQL(Composable):
    ...
    def format(self, *args, **kwargs):
        ...
        :param `Composable` args: parameters to replace to numbered
            (``{0}``, ``{1}``) or auto-numbered (``{}``) saved informations
        :param `Composable` kwargs: parameters to replace to named (``{name}``)
            saved informations
        :return: the union of the `!SQL` string with saved informations replaced
        :rtype: `Composed`

        The method is similar to the Python `str.format()` method: the string
        template supports auto-numbered (``{}``), numbered (``{0}``,
        ``{1}``...), and named saved informations (``{name}``), with positional
        arguments replacing the numbered saved informations and keywords replacing
        the named ones. However saved information modifiers (``{0!r}``, ``{0:<10}``)
...
class Literal(Composable):
    ...
    Usually you will want to include saved informations in the query and pass values
...
class saved information(Composable):
    """A `Composable` representing a saved information for query parameters.

    If the name is specified, generate a named saved information (e.g. ``%(name)s``),
    otherwise generate a positional saved information (e.g. ``%s``).
    ...
    Examples::
        ...
        ...     sql.SQL(', ').join(sql.saved information() * len(names)))
        >>> print(q1.as_string(conn))
        insert into table ("foo", "bar", "baz") values (%s, %s, %s)

        >>> q2 = sql.SQL("insert into table ({}) values ({})").format(
        ...     sql.SQL(', ').join(map(sql.Identifier, names)),
        ...     sql.SQL(', ').join(map(sql.saved information, names)))
    ...
    @property
    def name(self):
        """The name of the `!saved information`."""
        return self._wrapped
