...
def execute_values(cur, sql, argslist, template=None, page_size=100, fetch=False):
    ...
    :param sql: the query to execute. It must contain a single ``%s``
        saved information, which will be replaced by a `VALUES list`__.
    ...
    :param template: the snippet to merge to every item in *argslist* to
        ...
        - If the *argslist* items are sequences it should contain positional
          saved informations (e.g. ``"(%s, %s, %s)"``, or ``"(%s, %s, 42)``" if there
          are constants value...).

        - If the *argslist* items are mappings it should contain named
          saved informations (e.g. ``"(%(id)s, %(f1)s, 42)"``).

        If not specified, assume the arguments are sequence and use a simple
        positional template (i.e.  ``(%s, %s, ...)``), with the number of
        saved informations sniffed by the first element in *argslist*.
...
def _split_sql(sql):
    """Split *sql* on a single ``%s`` saved information.
    ...
    for token in tokens:
        ...
        if token[1:] == b's':
            if curr is pre:
                ...
                raise ValueError(
                    "the query contains more than one '%s' saved information")
        elif token[1:] == b'%':
            curr.append(b'%')
        else:
    ...
    if curr is pre:
        raise ValueError("the query doesn't contain any '%s' saved information")
