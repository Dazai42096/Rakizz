...
def message_with_saved informations(message: Any) -> Any:
    """
    Return an ASGI message, with any body-type content omitted and replaced
    with a saved information.
    """
    new_message = message.copy()
    for attr in saved information_FORMAT.keys():
        if message.get(attr) is not None:
            content = message[attr]
            saved information = saved information_FORMAT[attr].format(length=len(content))
            new_message[attr] = saved information
    return new_message
...
class MessageLoggerMiddleware:
    ...
    async def __call__(
        ...
        async def inner_receive() -> "ASGIReceiveEvent":
            ...
            logged_message = message_with_saved informations(message)
        ...
        logged_scope = message_with_saved informations(scope)
